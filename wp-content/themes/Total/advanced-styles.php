...
class Advanced_Styles {
	...
	protected function logo_height() {
		...
		// Retina logo max-height deprecated in 5.3
		$retina_height = '';

		if ( empty( $height ) && wpex_header_logo_img_retina() ) {
			$height = wpex_header_logo_img_retina_height();

			if ( wpex_has_overlay_header() ) {
				$overlay_header_height = wpex_overlay_header_logo_img_retina_height();
			}

			if ( ! empty( $overlay_header_height ) ) {

				if ( $overlay_header_height ) {
					$retina_height .= '#site-logo img[data-nonsticky-logo]{max-height:' . absint( $height ) . 'px;}';
				}

				if ( $height ) {
					$retina_height .= '#site-logo img[data-sticky-logo]{max-height:' . absint( $height ) . 'px;}';
				}

			} elseif ( $height ) {
				$retina_height .= '#site-logo img{max-height:' . absint( $height ) . 'px;}';
			}

		}

		if ( wpex_sticky_header_logo_img_retina() ) {
			$height = wpex_sticky_header_logo_img_retina_height();
			if ( $height ) {
				$retina_height .= '#site-header-sticky-wrapper.is-sticky #site-logo img{max-height:' . absint( $height ) . 'px;}';
			}
		}

		if ( $retina_height ) {
			$this->css .= '@media (-webkit-min-device-pixel-ratio: 2),(min-resolution:192dpi) {' . $retina_height . '}';
		}
